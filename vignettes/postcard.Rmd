...
## Method robust to model misspecification
...
> Note that predictions can be obtained in any way the user sees fit. Here we showcase a simple use of a `glm()`, but fx. `fit_best_learner()` could be used to fit a discrete super learner as the prediction model. Could also add steps to get out-of-sample (OOS) predictions (see examples).
...
## Methods specific to linear models
