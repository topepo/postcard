...
dat_treat <- data.frame(Y = y_treat, W = w, X = x, A = a)

truemean_notreat <- b0+b1*x
y_notreat <- rnorm(n, mean = truemean_notreat, sd = 1)
dat_notreat <- data.frame(Y = y_notreat, W = w, X = x)
```

> Note! We use the same data as was generated in the [README](https://github.com/NNpackages/PostCard#README) examples. Here we will also generate new data sets to show results for other families than the Gaussian using definitions from those examples.
...
dat_pois <- dat_treat %>% 
  dplyr::mutate(Y = rpois(n, lambda = exp(truemean_treat)))
...
##### Example of creating own list of learners
...
- `leaners`: The list of learners used
