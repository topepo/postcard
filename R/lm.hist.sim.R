...
lm.hist.sim <- function(data.list,
  ...
  if (method == "none") {
    ...
    # out <- foreach::foreach(k = 1:N.sim, .combine = rbind) %dopar% {
  ...
  if (method == "procova") {
    ...
    out <- future.apply::future_lapply(X = 1:N.sim, FUN = function(k) {
      ...
      res <- c(estimate, std.err, mod$test_margin, coverage, MSE, type1.err) %>% stats::setNames(nm = c("estimate", "std.err", "test_stat", "power", "coverage", "MSE", "type1.err"))
