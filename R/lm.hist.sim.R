...
#' @examples
#'
#' data <- sim.lm(N.sim = 10, N.hist.control = 100, N.hist.treatment = 100,
...
lm.hist.sim <- function(data.list,
  ...
  varnames <- colnames(data.list[[1]]$hist) %>% setdiff(outcome.var)
  ...
  if (method == "none") {
    ...
    out <- future.apply::future_lapply(X = 1:N.sim, FUN = function(k) {
      ...
      s1 <- summary(mod)
      crit.val.t <- attr(mod, "crit.val.t")
      ...
      res <- c(estimate, std.err, mod$test_margin, coverage, MSE, type1.err) %>% setNames(c("estimate", "std.err", "test_stat", "power", "coverage", "MSE", "type1.err"))
    ...
    out <- future.apply::future_lapply(X = 1:N.sim, FUN = function(k) {
      ...
      if (est.power) {
        if (is.null(data.list[[k]]$hist_test)) {
          ...
          hist$pred <- predict(attr(mod, "prediction_model"), hist)
        }
        prelim <- power.ancova(data.hist = hist, outcome.var = outcome.var, treatment.var = treatment.var,
      ...
      if (L2) {
        ...
        if (n1_hist == 0) {
          ...
          rct0$pred <- predict(attr(mod, "prediction_model"), rct0)
          X <- model.matrix(formula(paste0("y ~ -1 +", paste0("(", paste0("x", 1:N.covs, collapse = "+"), ")^2"),
                                           "+", paste0("I(x", 1:N.covs, "^2)", collapse = "+"))),
                            data = rct0)
          rct0$progscore <- X %*% c(rep(coefs[2], N.covs), rep(coefs[1], N.covs), rep(coefs[1], ncol(X) - (2*N.covs)))
          prelim <- c(mean((rct0$pred - rct0$progscore)^2)) %>% setNames(c("L2"))
          res <- c(res, prelim)
        } else {
          # Predict as if they receive treatment
          rct <- data.list[[k]]$rct %>% dplyr::mutate("w" = 1)
          rct$pred1 <- predict(attr(mod, "prediction_model"), rct)
          ...
          rct$pred0 <- predict(attr(mod, "prediction_model"), rct)
          X <- model.matrix(formula(paste0("y ~ -1 +", paste0("(", paste0("x", 1:N.covs, collapse = "+"), ")^2"),
                                           "+", paste0("I(x", 1:N.covs, "^2)", collapse = "+"))),
                            data = rct)
          ...
          prelim <- c(L2_1, L2_0) %>% setNames(c("L2_1", "L2_0"))
          res <- c(res, prelim)
        }
      }
      res
  ...
  if (method == "oracle0") {
    ...
    N.covs <- attr(data.list, "N.covs")
    coefs <- attr(data.list, "coefs")
    crit.val.t <- qt(1 - alpha/2, nrow(data.list[[1]]$rct) - 3 - length(adj.covs) - ifelse(interaction, (length(adj.covs) + 1), 0))
    ...
    out <- future.apply::future_lapply(X = 1:N.sim, FUN = function(k) {
      ...
      mod_procova <- lm(formula = formula.procova, data = rct.dm)
      ...
      res <- c(estimate, std.err, test_stat, power, coverage, MSE, type1.err) %>% setNames(c("estimate", "std.err", "test_stat", "power", "coverage", "MSE", "type1.err"))
  ...
  if (method == "oracle") {
    ...
    N.covs <- attr(data.list, "N.covs")
    coefs <- attr(data.list, "coefs")
    crit.val.t <- qt(1 - alpha/2, nrow(data.list[[1]]$rct) - 4 - length(adj.covs) - ifelse(interaction, (length(adj.covs) + 2), 0))
