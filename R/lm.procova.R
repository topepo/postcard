...
lm.procova <- function(data.list,
  ...
  if (method == "procova") {
    ...
    if (n1_hist == 0) {
      ...
      rct.dm <- cbind(rct.dm, pred)
      colnames(rct.dm) <- c(colname, "pred")
    ...
    if (n1_hist > 0) {
      ...
      pred1 <- stats::predict(prediction_model, new_data = rct, ...) %>% stats::setNames(nm = c("pred1"))
      rct.dm <- cbind(rct.dm, pred1)
      rct.dm$pred1 <- rct.dm$pred1 - mean(rct.dm$pred1)
      # Predict as if they receive control
      rct <- rct %>% dplyr::mutate("w" = 0)
      pred0 <- stats::predict(prediction_model, new_data = rct, ...) %>% stats::setNames(nm = c("pred0"))
      rct.dm <- cbind(rct.dm, pred0)
