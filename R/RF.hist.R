...
#'
RF.hist <- function(data.hist, outcome.var = "y", method.covs = c("x1", "x2")){

  k <- length(method.covs)
  formula_RF <- formula(paste0(outcome.var, " ~ ", paste0(method.covs, collapse = " + ")))
  ...
  rf1 <- parsnip::rand_forest(
    ...
    parsnip::fit(formula_RF , data.hist %>% dplyr::select(outcome.var, method.covs))
